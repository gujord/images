...
fetch --no-verify-peer -a https://rawgit.com/gujord/images/master/make-arm64.conf -o /etc/fabrik-arm64-make.conf
# fetch --no-verify-peer -a https://rawgit.com/gujord/images/master/fabrik-aws.kernel -o /usr/src/sys/amd64/conf/${KERNEL}
...
cd /usr/src
env MAKEOBJDIRPREFIX=${WRKDIR}/host/obj SRCCONF=/etc/fabrik-src.conf __MAKE_CONF=/etc/fabrik-aws-make.conf make -DNO_CLEAN -j${NUMBER_OF_CORES} TARGET_ARCH=${ARCH} buildworld
env MAKEOBJDIRPREFIX=${WRKDIR}/host/obj SRCCONF=/etc/fabrik-src.conf __MAKE_CONF=/etc/fabrik-aws-make.conf make -DNO_CLEAN -j${NUMBER_OF_CORES} TARGET_ARCH=${ARCH} buildkernel KERNCONF=${KERNEL}
...
cd /usr/src;
env MAKEOBJDIRPREFIX=${WRKDIR}/host/obj SRCCONF=/etc/fabrik-src.conf __MAKE_CONF=/etc/fabrik-aws-make.conf make DESTDIR=/mnt  TARGET_ARCH=${ARCH} installworld 2>&1 | tee ${LOGDIR}/host-installworld.log &&
env MAKEOBJDIRPREFIX=${WRKDIR}/host/obj SRCCONF=/etc/fabrik-src.conf __MAKE_CONF=/etc/fabrik-aws-make.conf make DESTDIR=/mnt  TARGET_ARCH=${ARCH} installkernel KERNCONF=${KERNEL} 2>&1 | tee ${LOGDIR}/host-installkernel.log &&
env MAKEOBJDIRPREFIX=${WRKDIR}/host/obj SRCCONF=/etc/fabrik-src.conf __MAKE_CONF=/etc/fabrik-aws-make.conf make DESTDIR=/mnt  TARGET_ARCH=${ARCH} distribution 2>&1 | tee ${LOGDIR}/host-distribution.log
